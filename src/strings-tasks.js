...
function removeFirstOccurrences(str, value) {
  ...
  return startIndex >= 0 ? str.slice(0, startIndex) + str.slice(endIndex) : str;
}
...
function countVowels(str) {
  return str
    .toLowerCase()
    .split('')
    .reduce((acc, cur) => ('aeiouy'.includes(cur) ? acc + 1 : acc), 0);
}
...
function isPalindrome(str) {
  const regex = /[ .,!?]/g;
  const cleanStr = str.toLowerCase().replaceAll(regex, '');
